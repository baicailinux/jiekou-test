...
def relust(titles, starttime, endtime, passge, fail, id, name, key, coneent, url, meth, yuqi, json, relust):
    if type(name) == list:
        ...
        for i in range(len(name)):
            relus += (
            ceshixiangqing(id[i], name[i], key[i], coneent[i], url[i], meth[i], int(yuqi[i]), json[i], relust[i]))
        text = title(titles) + connent + time(starttime, endtime, passge, fail) + shanghai + relus + weibu
    else:
        text = title(titles) + connent + time(starttime, endtime, passge, fail) + shanghai + ceshixiangqing(id, name,
                                                                                                            key,
                                                                                                            coneent,
                                                                                                            url, meth,
                                                                                                            int(yuqi),
